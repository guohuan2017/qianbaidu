...
<link rel="stylesheet" type="text/css"
	href="/qianbaidu/css/Hui-iconfont/1.0.8/iconfont.css" />
</head>
...
<body>
	<div id="head-bar">
		<div id="head-menu">
			<span class="carttopbar-nav-path"> 当前位置: <a
				class="carttopbar-nav-path2" href="">徐汇民润大厦(宜山路北50米)</a> <i
				class="Hui-iconfont">&#xe6d7;</i> <a class="carttopbar-nav-path2"
				href="">${store.storename}</a> <i class="Hui-iconfont">&#xe6d7;</i>
				订单信息
			</span>
			<div id="dlzc">
				<span class="dlzc1"> </span> <span> <a class="dlzc2">欢迎:${user.username}</a>
				</span>
			</div>
		</div>
	</div>
	<div id="head-div">
		<div id="head-info">
			<img class="checkout" src="/qianbaidu/img/logo/千百度logo_图片.png" style="width:50px; display:inline-block;position:relative;top:15px;"/>
			<span class="checkoutguide" style="display:inline-block;top:10px;margin-left:0;">千百度 &nbsp;订单信息 </span>
	...
	<div id="content-center">
		<div id="order-details">
			<div id="order-details-container">
						...
						<dd>
							<div class="checkoutcart-tablerow">
								...
								<div class="cell-itemquantity">
									...
									<button style="position: relative; left: 5px;"
										class="number-btn" id="minusbtn${food.id}">-</button>
									<input type="text" id="foodidnumber${food.id}"
										class="food-number" style="width: 40px" value="${food.number}"
										readonly="readonly" />
									<button style="position: relative; left: -5px;"
										class="number-btn" id="addbtn${food.id}">+</button>
									<%-- <button style="position: relative; left: -5px; " class="number-btn" id="addbtn${food.id}"
										ondblclick="add(foodid)">+</button> --%>
									<!--  -->
								</div>
								<div class="cell-binding1" id="price${food.id}"></div>
								<span style="display: none" id="foodprice${food.id}">${food.price}</span>
							</div>
						</dd>
						<script type="text/javascript">
							var minus = document
									.getElementById('minusbtn${food.id}');
							var add = document
									.getElementById('addbtn${food.id}');

							minus.onclick = function() {
								var num = document
										.getElementById('foodidnumber${food.id}');
								...
								var lineprice = document
										.getElementById('price${food.id}');

								var price = document
										.getElementById('foodprice${food.id}');
				...
				<div class="cell-jiesuan">
					￥ <span class="cell-jine" id="totalprice"> ${total} </span>
				</div>
				<div class="cell-jiesuan2">
					共 <span class="cell-jine2" id="totalnumber"> ${num} </span> 份商品
				</div>
			</div>
			...
			<script type="text/javascript">
				...
				div.onclick = function() {
					...
					var nums = document.getElementsByClassName('food-number');
					var prices = document
							.getElementsByClassName('cell-binding1');
		...
		<div id="other-info">
			<div class="checkout-shdz">
				<h2 class="checkout-fkfs-title">
					收获地址 <a class="checkout-tjdz" href="">添加新地址</a>
				</h2>
				<ul>
					<li class="checkout-tjdz-bk"><i class="Hui-iconfont"
						style="vertical-align: middle; width: 63px; height: 42px; display: table-cell; border-right: 1px solid #eee; font-size: 30px; text-align: center; color: #999999;">&#xe6c9;</i>
			...
			<div class="checkout-fkfs">
				<h2 class="checkout-fkfs-title">
					付款方式 <span class="checkout-fkfs-title1">推荐使用在线支付，不用找零，优惠更多</span>
				</h2>
				<ul>
			...
			<div class="checkout-xzyh">
				<h2 class="checkout-fkfs-title">选择优惠</h2>
				<p class="xzyh-ng-binding">
					<span class="xzyh-ng-span">使用红包</span> <span
						style="color: #999999;">无可用红包</span>
				</p>
				<p class="xzyh-ng-binding">
					<span class="xzyh-ng-span">使用优惠券</span> <span
						style="color: #999999;">网站不支持 <em
						style="color: red; font-style: normal;">（仅手机客户端可用）</em>
					</span>
				</p>
			</div>
			<div class="checkout-qtxx">
				<h2 class="checkout-fkfs-title">其他信息</h2>
				<p class="xzyh-ng-binding">
					<span class="xzyh-ng-span">配送方式</span> <span>本订单由 <a
						style="color: #0089dc; font-style: normal">[蜂鸟专送]</a> 提供配送
					</span>
				</p>
				<p class="xzyh-ng-binding">
					<span class="xzyh-ng-span">送达时间</span> <select
				...
				<p class="xzyh-ng-binding">
					<span class="xzyh-ng-span">订单备注</span> <input class="qtxx-ng-input"
				...
				xiadan.onclick = function() {
					...
					for (var i = 0; i < nums.length; i++) {
						...
						if (num > 0) {
							total.value += price;
							number.value += num;
							order = order + parseInt(foodids[i].innerHTML)
									+ "_" + num + ";";
						}
					}
